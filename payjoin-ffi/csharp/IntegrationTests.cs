...
namespace Payjoin.Tests
{
    ...
    public class IntegrationTests : IAsyncLifetime
    {
            ...
            TransitionMode mode)
        {
            ...
            {
                var taggedRefs = proposal.GetInputScriptRefs()
                    .Select(r => r.Mark(new IsScriptOwnedCallback(receiverRpc).Callback(r.GetValue())))
                    .ToArray();
                using var transition = proposal.ApplyInputOwnedChecks(taggedRefs);
                maybeInputsSeen = transition.Save(recvPersister);
            }
            ...
            TransitionMode mode)
        {
            ...
            {
                var taggedRefs = proposal.GetInputOutpointRefs()
                    .Select(r => r.Mark(new CheckInputsNotSeenCallback().Callback(r.GetValue())))
                    .ToArray();
                using var transition = proposal.ApplyInputSeenChecks(taggedRefs);
                outputsUnknown = transition.Save(recvPersister);
            }
            ...
            TransitionMode mode)
        {
            ...
            {
                var taggedRefs = proposal.GetOutputScriptRefs()
                    .Select(r => r.Mark(new IsScriptOwnedCallback(receiverRpc).Callback(r.GetValue())))
                    .ToArray();
                using var transition = proposal.ApplyOutputOwnedChecks(taggedRefs);
                wantsOutputs = transition.Save(recvPersister);
            }
