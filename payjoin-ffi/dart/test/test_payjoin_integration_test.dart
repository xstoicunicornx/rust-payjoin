...
Future<payjoin.PayjoinProposalReceiveSession> process_outputs_unknown(
  ...
  if (mode == TransitionMode.callback) {
    ...
    final tagged_refs = proposal
        .getOutputScriptRefs()
        .map(
          (ref) => ref.mark(
            result: IsScriptOwnedCallback(receiver).callback(ref.getValue()),
          ),
        )
        .toList();
    wants_outputs = proposal
        .applyOutputOwnedChecks(checkedOutputScripts: tagged_refs)
        .save(persister: recv_persister);
  }
...
Future<payjoin.PayjoinProposalReceiveSession> process_maybe_inputs_seen(
  ...
  if (mode == TransitionMode.callback) {
    ...
    final tagged_refs = proposal
        .getInputOutpointRefs()
        .map(
          (ref) => ref.mark(
            result: CheckInputsNotSeenCallback(
              receiver,
            ).callback(ref.getValue()),
          ),
        )
        .toList();
    outputs_unknown = proposal
        .applyInputSeenChecks(checkedInputOutpoints: tagged_refs)
        .save(persister: recv_persister);
  }
...
Future<payjoin.PayjoinProposalReceiveSession> process_maybe_inputs_owned(
  ...
  if (mode == TransitionMode.callback) {
    ...
    final tagged_refs = proposal
        ...
        .toList();
    maybe_inputs_owned = proposal
        .applyInputOwnedChecks(checkedInputScripts: tagged_refs)
        .save(persister: recv_persister);
  }
