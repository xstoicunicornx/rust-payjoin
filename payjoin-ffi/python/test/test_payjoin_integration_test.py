...
class TestPayjoin(unittest.IsolatedAsyncioTestCase):
    ...
    async def process_maybe_inputs_owned(
        ...
        if mode == "callback":
            ...
            tagged_refs = [
                ref.mark(IsScriptOwnedCallback(self.receiver).callback(ref.get_value()))
                for ref in proposal.get_input_script_refs()
            ]
            maybe_inputs_owned = proposal.apply_input_owned_checks(tagged_refs).save(
                recv_persister
            )
        return await self.process_maybe_inputs_seen(
    ...
    async def process_maybe_inputs_seen(
        ...
        if mode == "callback":
            ...
            tagged_refs = [
                ref.mark(
                    CheckInputsNotSeenCallback(self.receiver).callback(ref.get_value())
                )
                for ref in proposal.get_input_outpoint_refs()
            ]
            outputs_unknown = proposal.apply_input_seen_checks(tagged_refs).save(
                recv_persister
            )
    ...
    async def process_outputs_unknown(
        ...
        if mode == "callback":
            ...
            tagged_refs = [
                ref.mark(IsScriptOwnedCallback(self.receiver).callback(ref.get_value()))
                for ref in proposal.get_output_script_refs()
            ]
            wants_outputs = proposal.apply_output_owned_checks(tagged_refs).save(
                recv_persister
            )
