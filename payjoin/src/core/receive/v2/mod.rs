...
impl Receiver<Monitor> {
    ...
    pub fn check_payment(
        ...
        match transaction_exists(self.extract_payjoin_proposal_txid()) {
        ...
        match transaction_exists(self.extract_fallback_txid()) {
    ...
    pub fn extract_fallback_tx(&self) -> Transaction {
        self.state.psbt_context.original_psbt.clone().extract_tx_unchecked_fee_rate()
    }

    pub fn extract_fallback_txid(&self) -> Txid { self.extract_fallback_tx().compute_txid() }
    ...
    pub fn check_fallback_monitorable(
        ...
        if has_empty_witness(&self.extract_fallback_tx()) {
